...
def compute_img_mean_std(image_paths):
    ...
    """
    path = os.path.join(image_paths,'HAM')
    all_image_path_dmf = glob(os.path.join(path,'*.jpg'))

    img_h, img_w = args.img_size, 224
    imgs = []
    means, stdevs = [], []
...
def create_dataframes(data_dir_dmf):
    path = os.path.join(data_dir_dmf,'HAM')
    all_image_path_dmf = glob(os.path.join(path,'*.jpg'))
    ...
    df_original = pd.read_csv('/home/jfayyad/PycharmProjects/BiasWashV2/dataverse_files/HAM10000_metadata')
